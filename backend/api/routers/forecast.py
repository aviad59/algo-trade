...
from ..services.forecast import build_material_forecast, build_ranking, build_summary

router = APIRouter(prefix="/forecast", tags=["forecast"])


@router.get("/summary")
def forecast_summary(buf: Buffer = Depends(get_buffer)) -> dict:
...
@router.get("/ranking")
def forecast_ranking(buf: Buffer = Depends(get_buffer)) -> dict:
    settings = get_settings()
