...
Each CLI has two forms — the short `algo-trade-*` shortcut and the module invocation. Use the second if the shortcut isn't on your PATH (common on Windows Store Python).

| Purpose | Shortcut | Module form |
|---|---|---|
| Fetch SEC filings to stdout (JSONL) | `algo-trade-fetch` | `python -m algo_trade.fetcher` |
| Fetch → extract → upsert into buffer | `algo-trade-extract` | `python -m algo_trade.extract_cli` |
| Render material forecast curve (PNG / HTML) | `algo-trade-plot` | `python -m algo_trade.plot_cli` |
| Start the FastAPI backend | `algo-trade-api` | `python -m api.main` |

Both forms run the exact same code — the shortcut is generated by `pip install -e .` and dropped in your Python's Scripts folder. If PowerShell reports `'algo-trade-extract' is not recognized`, the Scripts folder isn't on your PATH; the module form always works.
...
# or equivalently:
python -m algo_trade.extract_cli TSLA GM \
    --identity "Your Name you@example.com" \
    --form 10-Q --limit 1
```

### Getting the shortcuts on PATH (Windows Store Python)

If you want the short names to work in every terminal, add the Scripts folder to your user PATH once:

```powershell
$scripts = "$env:LOCALAPPDATA\Packages\PythonSoftwareFoundation.Python.3.13_qbz5n2kfra8p0\LocalCache\local-packages\Python313\Scripts"
...
Close and reopen PowerShell — `algo-trade-*` will resolve in every new terminal after that. If your Python version is different, adjust `Python313` accordingly (`python -c "import site; print(site.USER_BASE)"` will tell you the base path).
...
2. With the dev stack already running, open a **second terminal** at the repo root and run:
   ...
   ```
   If PowerShell says `algo-trade-extract is not recognized`, your Python's Scripts folder isn't on PATH (common on Windows Store Python). Same command using the module invocation, which always works:
   ```powershell
   python -m algo_trade.extract_cli TSLA GM FCX --form 10-Q --limit 1 -v
   ```
   Every `algo-trade-*` CLI has an equivalent `python -m ...` form — see the [CLI commands](#cli-commands) section.
